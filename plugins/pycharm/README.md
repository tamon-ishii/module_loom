...
同じ解析対象を MkDocs マニュアルにするには、**MkDocs 出力** を押して出力先ディレクトリを選択します。空のディレクトリ、または ModuleLoom が以前生成したディレクトリを指定できます。新規作成時はプロジェクト直下に `moduleloom-docs` などの空フォルダを作成して選んでください。生成後はそのフォルダで `mkdocs serve` を実行します。表示には `mkdocs-material` が必要です。
