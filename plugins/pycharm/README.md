...
画面の **表示 → 言語** で日本語と英語を切り替えられます。初回は環境の言語を使い、選択した言語は次回起動時にも保持されます。PyCharm 上部の操作ボタンと右クリックメニューにも選択を反映します。
...
人が編集する手順書を作る場合は **ドキュメント生成** タブを使います。原稿内の `ai:task` に基づき、**全スクショ一括撮影** で実画面を自動巡回キャプチャ（赤枠・赤丸ハイライト対応）、**全ダイアグラム更新** でMermaid依存図を一括生成、**APIドキュメント生成** でDocstringリファレンスをワンクリック出力します。各アセットはサムネイル一覧でプレビューでき、気になった画像のみリテイク・修正指示を出せます。承認ボタンの個別押しは不要で、画像や図が揃っていればそのまま完成版サイトに反映されます。HTMLサイトの出力には端末に `mkdocs` コマンドが必要です。
