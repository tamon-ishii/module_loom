...
同じ解析対象を MkDocs マニュアルにするには、**MkDocs 出力** を押して出力先ディレクトリと言語を選択し、表示される生成先・モジュール数・言語を確認します。`auto` はマシンのロケールを使います。空のディレクトリ、または ModuleLoom が以前生成したディレクトリを指定できます。新規作成時はプロジェクト直下に `moduleloom-docs` などの空フォルダを作成して選んでください。生成物には API 一覧とモジュール別ページが含まれ、Mermaid のスクリプトを同梱するためオフラインでも図を表示できます。生成後はそのフォルダで `mkdocs serve` を実行します。表示には `mkdocs-material` が必要です。
...
ビルドには JDK 21 と、`jbr/bin/javac` および `lib/*.jar` を含む展開済み PyCharm が必要です。`PYCHARM_HOME` で PyCharm の場所を指定して JAR をビルドします。
